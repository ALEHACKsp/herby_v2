...
ConCommandBase *ConCommandBase::s_pRegisteredCommands = nullptr;
...
public:
	virtual bool RegisterConCommandBase(ConCommandBase *pVar)
	{
		...
		csgo::m_cvar->RegisterConCommand(pVar);
		return true;
	}
};
...
{
	if (!csgo::m_cvar || s_bRegistered)
		return;

	assert(s_nDLLIdentifier < 0);
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = csgo::m_cvar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	csgo::m_cvar->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if (csgo::m_cvar) {
		csgo::m_cvar->UnregisterConCommand(this);
	}
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
	...
	if (csgo::m_cvar)
		csgo::m_cvar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
